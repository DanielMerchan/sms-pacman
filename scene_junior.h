...
void set_junior(void) {
    ...
    play_track(AUDIO_JUNIOR, 0);
	SMS_displayOn();
}

// Updates stork sprite
void update_stork(void) {
    // If stork reched end of screen, return
    if (EntityX[INDEX_MISC] == 0) {
        return;
    }
    // Draw stork sprite
    SMS_addMetaSprite(EntityX[INDEX_MISC] -= get_entity_move_speed(INDEX_MISC), EntityY[INDEX_MISC], stork_frames[EntityFrame[INDEX_MISC]]);
...
void update_baby_pac(void) {
    // Follow stork
    if (SubState < 3) {
        SMS_addMetaSprite(EntityX[INDEX_MISC] - 4, EntityY[INDEX_MISC] - 2, stork_bag);
    // Falling
    } else if (SubState == 3) {
        unsigned char speed = get_entity_move_speed(INDEX_BONUS);
...
void update_junior_entities(void) {
...
void update_junior(void) {
	// Update entities
	update_junior_entities();
	switch (SubState) {
		// Clacker
		case 0:
            // Act number sprite
            update_act_number(101, 79);
