...
// HUD slots / Indexes taken from SMSTileStudio in the bottom of the tile map area
...
void set_hud_slot(unsigned char slot, unsigned char item) {
...
// Sets lives on HUD
void set_hud_lives(unsigned char lives, unsigned char item) {
	...
	for (unsigned char i = 0; i < 5; i++) {
		...
		// Slot in an empty
			...
			set_hud_slot(i, HUD_EMPTY);
		}
	}
}

// Sets bonus items on HUD
void set_bonus_items(void) {
...
// Using an unroll loop for performance!!!!!
void add_amount(unsigned char amountIndex, unsigned char offset) {
	// Local vars
...
// Updates top portion of the HUD in VRAM using UNSAFE method
void update_hud_top(void) {
...
void update_hud_bottom(void) {
	// Update bottom of HUD
	if (CurrentPlayer == INDEX_PLAYER_01) {
		set_hud_lives(P1Lives, HUD_MS_PAC_MAN_P1);
		// Update display if visible
		if (ScrollY >= 80) {
			SMS_loadTileMapArea(0, get_row(34), &P1Map[1088], 32, 2);
		}
	} else {
		set_hud_lives(P2Lives, HUD_MS_PAC_MAN_P2);
		// Update display if visible
		if (ScrollY >= 80) {
			SMS_loadTileMapArea(0, get_row(34), &P2Map[1088], 32, 2);
		}
	}
}

// Updates the score with the given points constant eg. POINTS_10
void add_score(unsigned char index, unsigned char amountIndex) {
	...
	if (index == INDEX_PLAYER_01) {
		...
		if (P1Score < ExtraLife && P1Score + hud_points[amountIndex] >= ExtraLife) {
			P1Lives++;
			update_hud_bottom();
		    play_track(AUDIO_EXTRA_LIFE, 0);
		}
		P1Score += hud_points[amountIndex];
		add_amount(amountIndex, 3);
	} else {
		// If under the extra life score, but will earn an extra life with points added
