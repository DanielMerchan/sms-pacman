...
// Sets ghost data
void set_ghost(unsigned char index, unsigned char dir, unsigned char state, unsigned short x, unsigned short y) {
    EntityX[index] = x;
    EntityY[index] = y;
    EntityDir[index] = dir;
...
void update_ghost_frame(unsigned char index) {
...
void reset_ghost_frame(unsigned char index) {
    EntityFrame[index] = 0;
    EntityFrameDuration[index] = -1;
}

// Sets the given entity's reverse direction
void set_ghost_reverse_direction(unsigned char index) {
    // Reset frame
    reset_ghost_frame(index);
...
void set_ghost_model(unsigned char ghostMode) {
    ...
    GhostMode = ghostMode;
    set_all_ghost_states(GhostMode);
}

// Checks for player collisions
void ghost_check_collision(unsigned char index) {
	...
	unsigned char target = check_entity_collision(index);
...
void set_random_ghost_direction(unsigned char index) {
...
void set_ghost_direction(unsigned char index, unsigned short target) {
    // Set available ghost directions
    set_ghost_available_directions(EntityTile[index], EntityDir[index]);
    ...
    if (GhostDirs[DIRECTION_RIGHT] > 0) {
        dist = get_distance(EntityTile[index] + 1, target);
        if (dist <= minDist) {
            minDist = dist;
    ...
    if (GhostDirs[DIRECTION_DOWN] > 0) {
        dist = get_distance(EntityTile[index] + 32, target);
        if (dist <= minDist) {
            minDist = dist;
...
void ghost_move_towards_target(unsigned char index, unsigned short target) {
    // Set target
    EntityTargetTile[index] = target;
    // Check for player collisions
    ghost_check_collision(index);
    // Check screen wraps
    check_wrap(index);
    // If setting random direction
    if (target == 65535) {
        set_random_ghost_direction(index);
    } else {
        set_ghost_direction(index, target);
    }
    // Update previous tile position
    EntityPrevTile[index] = EntityTile[index];
    // Snap to mid tile
    entity_snap_title(index, EntityDir[index]);
    // Move to available direction
    move_entity(index, EntityDir[index]);
}

// Processes ghost in house state
void ghost_house(unsigned char index) {
...
void ghost_entering(unsigned char index) {
...
void ghost_points(unsigned char index) {
    // Wait, show points
...
void blinky_chase(unsigned char target) {
    ghost_move_towards_target(INDEX_BLINKY, EntityTile[target]);
}

// Pinky chase behavior
void pinky_chase(unsigned char target) {
    // Get four tiles ahead of player position / direction
    switch (EntityDir[target]) {
        case DIRECTION_DOWN: ghost_move_towards_target(INDEX_PINKY, EntityTile[target] + 128); return;
        case DIRECTION_LEFT: ghost_move_towards_target(INDEX_PINKY, EntityTile[target] - 4); return;
        case DIRECTION_UP: ghost_move_towards_target(INDEX_PINKY, EntityTile[target] - 128); return;
        default: ghost_move_towards_target(INDEX_PINKY, EntityTile[target] + 4); return;
    }
}

// Inky chase behavior
void inky_chase(unsigned char target) {
...
// Sue chase behavior
void sue_chase(unsigned char target) {
    // If 8 tiles outside of player, target player
    if (get_distance(EntityTile[INDEX_SUE], EntityTile[target]) >= 64) {
        ghost_move_towards_target(INDEX_SUE, EntityTile[target]);
    // Target scatter tile
    } else {
        ghost_move_towards_target(INDEX_SUE, ScatterTargets[INDEX_SUE]);
    }
}

// Gets target index based on game mode and current player
unsigned char get_ghost_target_index(unsigned char index) {
    // If game mode is team, get random
    if (GameMode == GAME_MODE_TEAM_PLAY) {
        return get_rng(7, index) + INDEX_PLAYER_01;
    }
...
unsigned char ghost_position_changed(unsigned char index) {
...
void ghost_frightened(unsigned char index) {
    ...
    if (ghost_position_changed(index) == 0) {
        return;
    }
    // Move in random direction
    ghost_move_towards_target(index, 65535);
}

// Processes chomped state
void ghost_chomped(unsigned char index) {
    ...
    ghost_move_towards_target(index, 463);
}

// Processes scatter state
void ghost_scattering(unsigned char index) {
    // If no change in tile position, skip targeting logic
    if (ghost_position_changed(index) == 0) {
        return;
    }
...
void ghost_chasing(unsigned char index) {
    // If no change in tile position, skip targeting logic
    if (ghost_position_changed(index) == 0) {
        return;
    }
    // Get target tile index
    unsigned char target = get_ghost_target_index(index);
    // Set target based on ghost
    switch (index) {
        case INDEX_BLINKY: blinky_chase(target); break;
        case INDEX_PINKY: pinky_chase(target); break;
        case INDEX_INKY: inky_chase(target); break;
        case INDEX_SUE: sue_chase(target); break;
    }
}

#ifdef TIME_NTSC
    // Sets ghost speed based on state and stage number
    void set_ghost_speed(unsigned char index) {
        ...
        } else if (get_map_tile_id(EntityTile[index]) == TILE_TUNNEL) {
            EntitySpeed[index] = 0;
...
void UpdateGhostState(unsigned char index) {
    ...
    EntityTile[index] = position_to_tile_offset(index);
    ...
    // Set speed
    set_ghost_speed(index);
	// Update ghost state
	switch (state) {
        case ENTITY_STATE_HOUSE: ghost_house(index); break;
        case ENTITY_STATE_LEAVING: ghost_leaving(index); break;
        case ENTITY_STATE_ENTERING: ghost_entering(index); break;
        case ENTITY_STATE_SCATTERING: ghost_scattering(index); break;
        case ENTITY_STATE_CHASING: ghost_chasing(index); break;
        case ENTITY_STATE_FRIGHTENED: ghost_frightened(index); break;
        case ENTITY_STATE_CHOMPED: ghost_chomped(index); break;
        case ENTITY_STATE_POINTS: ghost_points(index); break;
	}
}

// Updates ghost
void update_ghost(unsigned char index) {
    update_ghost_sprites(index);
    update_ghost_state(index);
    update_ghost_frame(index);
}
