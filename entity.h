...
// returns true if the entity hits the threshold
unsigned char entity_increment_timer(unsigned char index, unsigned short threshold) {
    EntityTimer[index]++;
...
// Set the given entity's visibility on the viewport
void set_entity_visibility(unsigned char index) {
...
// Sets the entity move speed
void set_entity_move_speed(unsigned char index, unsigned char speed, unsigned char subSpeed) {
    EntitySpeed[index] = speed;
    EntitySubSpeed[index] = subSpeed;
}

// Get the speed of the entity based on subspeed increments
unsigned char get_entity_move_speed(unsigned char index) {
    // Get current speed
...
unsigned char check_entity_collision(unsigned char index) {
...
// Get the tile index based on the pixel coordinates given
unsigned short position_to_tile(unsigned short x, unsigned short y) {
    return ((y >> 3) << 5) + (x >> 3);
}

// Gets the tile index based on centered offsets
unsigned short position_to_tile_offset(unsigned char index) {
...
void set_entity_position_by_tile_index(unsigned char index, unsigned short tileIndex) {
...
unsigned char entity_can_move(unsigned short tile, unsigned char dir) {
	// Check if tile is solid
	switch (dir) {
		case DIRECTION_DOWN: return get_map_tile_id(tile + 32) > TILE_SOLID ? 0 : 1;
		case DIRECTION_LEFT: return get_map_tile_id(tile - 1) > TILE_SOLID ? 0 : 1;
		case DIRECTION_UP: return get_map_tile_id(tile - 32) > TILE_SOLID ? 0 : 1;
		default: return get_map_tile_id(tile + 1) > TILE_SOLID ? 0 : 1;
	}
}

// Moves the given entity by their movement speed
void move_entity(unsigned char index, unsigned char dir) {
...
void entity_snap_title(unsigned char index, unsigned char dir) {
...
void check_wrap(unsigned char index) {
	if (EntityX[index] < 8) {
		EntityX[index] = 240;
